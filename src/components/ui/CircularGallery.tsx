...
function createTextTexture(gl: GL, text: string, font: string = 'bold 30px monospace', color: string = 'white') {
  ...
  // Measure title
  ctx.font = font;
  const titleWidth = Math.ceil(ctx.measureText(text).width);

  // Measure CTA
  const ctaFont = `600 ${ctaFontSize}px sans-serif`;
  ctx.font = ctaFont;
  const ctaText = 'Guarda Demo →';
  const ctaTextWidth = Math.ceil(ctx.measureText(ctaText).width);
  const ctaBtnWidth = ctaTextWidth + 32;

  const canvasWidth = Math.max(titleWidth, ctaBtnWidth) + padding * 2;
  const canvasHeight = fontSize * 1.4 + lineGap + ctaHeight + padding * 2;
  ...
  ctx.fillText(text, canvasWidth / 2, padding + fontSize * 0.7);

  // CTA pill background
  const btnX = (canvasWidth - ctaBtnWidth) / 2;
  const btnY = padding + fontSize * 1.4 + lineGap;
  const btnR = ctaHeight / 2;

  ctx.beginPath();
  ctx.roundRect(btnX, btnY, ctaBtnWidth, ctaHeight, btnR);
  ...
  ctx.textBaseline = 'middle';
  ctx.textAlign = 'center';
  ctx.fillText(ctaText, canvasWidth / 2, btnY + ctaHeight / 2);
...
class Title {
  ...
  createMesh() {
    ...
    const textHeightScaled = this.plane.scale.y * 0.42;
    const textWidthScaled = textHeightScaled * aspect;
    this.mesh.scale.set(textWidthScaled, textHeightScaled, 1);
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeightScaled * 0.5 - 0.18;
    this.mesh.setParent(this.plane);
  }
}

class Media {
  extra = 0; geometry: any; gl: GL; image: string; index: number; length: number;
  renderer: Renderer; scene: any; screen: any; text: string; viewport: any;
  ...
  width!: number; widthTotal!: number; x!: number; speed = 0;
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }: any) {
    ...
    this.screen = screen; this.text = text; this.viewport = viewport;
...
class App {
  ...
  planeGeometry!: Plane; medias: Media[] = []; mediasImages: any[] = [];
  ...
  constructor(container: HTMLElement, { items, bend = 1, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 }: any) {
    ...
    this.onResize(); this.createGeometry(); this.createMedias(items, bend, textColor, borderRadius, font);
